...
export async function GET(request: Request, context: { params: Promise<{ page: string }> }) {
    ...
    // Fallback if page is NaN
    if (isNaN(page) || page < 1) {
        return new Response('Invalid page', { status: 400 });
    }

    // Getting stores is usually needed to resolve usernames if product structure spans store
    // But data.ts uses getCachedProducts which returns all products.
    // If we need store username, we need getCachedStores
